...
function Operador() {
    ...
    const toggleProduct = (productName) => {
        setSelectedProducts(prevSelected => {
            const productExists = prevSelected.find(item => item.name === productName);
            if (productExists) {
    ...
    const isFormValid = selectedProducts.length > 0 && quantity && paymentMethod && clientData.gender && clientData.socioeconomicClass && clientData.ageGroup && clientData.purchaseFrequency;
        ...
        <div className="p-8 min-h-screen ">
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-lg">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto">
                            <label className="text-sm text-gray-700">Produtos</label>
                            {productList.map((productItem, index) => (
                                <div key={index} className="flex items-center space-x-2">
                                    <input
                                        ...
                                        className="w-4 h-4"
                                    />
                                    <label className="text-sm text-gray-600">{productItem.name} - R$ {productItem.price.toFixed(2)}</label>
                                </div>
                            ))}
                        </div>
                ...
                <div className="text-right mb-6">
                    <p className="text-lg font-semibold text-gray-700">Total: R$ {total.toFixed(2)}</p>
                </div>
